        ...
        for (int l = 1; l < Math.max(pgs.getHeight(), pgs.getWidth()); l++) {
            for (int side = 0; side < 4; side++) {
                switch (side) {
                    ...
                    case 1://right
                        x = desiredX + l;
                        // desiredX may be negative (callers offset it away from a resource patch,
                        // e.g. EconomyRush passes resourceX-1), so the lower bound needs checking
                        // here too, not just the upper one this side moves toward.
                        if (x < 0 || x >= pgs.getWidth()) {
                            continue;
                        }
...
//        System.out.println("buildIfNotAlreadyBuilding: action = " + action);
        if (!(action instanceof Build) || ((Build) action).type != type) {
            ...
            if (pos < 0 || pos >= pgs.getWidth() * pgs.getHeight()) {
                return false;
            }
