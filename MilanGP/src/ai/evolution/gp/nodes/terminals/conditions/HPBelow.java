...
public class HPBelow extends BoolNode implements PerturbableTerminal {
    public static final String NAME = "HPBelow";
    private static final double MIN_FRACTION = 0.05;
    private static final double MAX_FRACTION = 0.95;
    ...
    @Override
    public BoolNode copy() { return new HPBelow(fraction); }

    @Override
    public GPNode perturb(Random rnd) {
        double perturbed = fraction + (rnd.nextDouble() * 2 - 1) * MAX_DELTA;
        return new HPBelow(Math.max(MIN_FRACTION, Math.min(MAX_FRACTION, perturbed)));
    }
}
